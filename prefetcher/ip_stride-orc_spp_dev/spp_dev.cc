...
void spp_dev_prefetcher_initialize() {}

void spp_dev_prefetcher_cycle_operate() {}

uint32_t spp_dev_prefetcher_cache_operate(CACHE* cache, uint64_t addr, uint64_t ip, uint8_t cache_hit, uint8_t type, uint32_t metadata_in)
{
  uint64_t page = addr >> LOG2_PAGE_SIZE;
  uint32_t page_offset = (addr >> LOG2_BLOCK_SIZE) & (PAGE_SIZE / BLOCK_SIZE - 1), last_sig = 0, curr_sig = 0, confidence_q[cache->MSHR_SIZE], depth = 0;

  int32_t delta = 0, delta_q[cache->MSHR_SIZE];

  for (uint32_t i = 0; i < cache->MSHR_SIZE; i++) {
    confidence_q[i] = 0;
    delta_q[i] = 0;
  }
  confidence_q[0] = 100;
      ...
      if (confidence_q[i] >= PF_THRESHOLD) {
        ...
        if ((addr & ~(PAGE_SIZE - 1)) == (pf_addr & ~(PAGE_SIZE - 1))) { // Prefetch request is in the same physical page
          if (FILTER.check(pf_addr, ((confidence_q[i] >= FILL_THRESHOLD) ? SPP_L2C_PREFETCH : SPP_LLC_PREFETCH))) {
            cache->prefetch_line(ip, addr, pf_addr, (confidence_q[i] >= FILL_THRESHOLD), 0); // Use addr (not base_addr) to obey the same physical page boundary
...
uint32_t spp_dev_prefetcher_cache_fill(uint64_t addr, uint32_t set, uint32_t match, uint8_t prefetch, uint64_t evicted_addr, uint32_t metadata_in)
{
...
void spp_dev_prefetcher_final_stats() {}
